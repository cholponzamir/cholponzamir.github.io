...
router.post("/task", (req, res) => {
  ...
  var sql =
    "INSERT INTO tasks (taskName, dueDate, groupId, personId, priority, complete) VALUES (?,?,?,?,?,?)";
  var params = [
    task.name,
    task.dueDate,
    task.groupId,
...
router.get("/task", (req, res) => {
  ...
  let sql =
    "select tasks.id, taskName, dueDate, complete, priority, firstName, personId, groupId, groups.name as groupName" +
...
router.put("/task", (req, res) => {
  ...
  console.log(
    "data.id:" + data.id + " name:" + data + " due_date:" + data.dueDate
  );
  if (!data.id) {
    return res.status(400).send("Missing URL parameter id");
  }
